...
修一个客户端。裸关连接对任何 OpenAI 兼容客户端都是协议异常，Codex、其他工具同样
会受影响；代理侧补帧则一次修好所有客户端。两者不冲突，客户端配置可作为额外一层。
...
**把 `ResponseHeaderTimeout` 一并降到看门狗量级。** 修的是另一个洞（等响应头
阶段未被看门狗覆盖），与本次「收尾语义」是两件事，避免把两个独立改动混在一起。
...
端到端在 WSL 上用部署后的二进制验证：模拟「发部分内容后只发心跳」的上游，
客户端收到的字节以 `finish_reason":"timeout"` + `data: [DONE]` 收尾；模拟
「发 `finish_reason=length` 后挂死」的上游，只补出 `[DONE]`，`finish_reason`
保持 `length`、`timeout` 出现 0 次。
